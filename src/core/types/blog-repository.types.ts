...
export interface IBlogRepository {
  ...
  createPost(post: Omit<BlogPost, 'id' | 'createdAt' | 'updatedAt'>): Promise<RepositoryResult<BlogPost>>;
  ...
  getUserPosts(username: string, options?: BlogSearchOptions): Promise<RepositoryResult<BlogPostSummary[]>>;
  getUserDrafts(username: string): Promise<RepositoryResult<BlogPostSummary[]>>;
  getCurrentUserPosts(status?: 'draft' | 'published', page?: number, limit?: number): Promise<RepositoryResult<BlogPostSummary[]>>;

  // Search operations
  searchPosts(query: string, options?: BlogSearchOptions): Promise<RepositoryResult<BlogPostSummary[]>>;
